...
def impact_parameter_and_lensing_angle(geodesic, impact_parameter, m_prime, m0, mf):
    ...
    initial_conditions_0 = [0, impact_parameter, 0.0001, 0, impact_parameter/np.sqrt(2), 0, 1, 0]
    
    # the expected projection for vaidya geodesic
    initial_trajectory = plot_until_xstop(geodesic, initial_conditions_0, 100, m_prime, m0, mf)
    ...
    # Save fig
    #fig.savefig(f"Trajectory of lensing particle at impact parameter = {impact_parameter}.png", dpi=300, bbox_inches='tight')
    
    # calculate angle
    angle = np.arctan(initial_trajectory[2][-1]/initial_trajectory[0][-1])
    return np.abs(angle)
...
def plot_image(radius, points, x, theta, v0, m_prime, m0, mf, geodesic, directory, case_color):
    ...
    ax3.plot(x_new, y_new, color = case_color)  # Interpolated points
